/**************************************************************************
    ...
    $Id: deviceman.cc 728853 2007-10-24 13:09:41Z ossi $
...
#if defined(HAVE_LIBASOUND2) || defined(HAVE_LIBASOUND)
...
#else
#error HAVE_ALSA_ASOUNDLIB_H or HAVE_SYS_ASOUNDLIB_H should be defined
#endif
...
DeviceManager::DeviceManager(int def)
{
  ...
  if (def==-1)
  {
    KComponentData *tmp_instance = 0;
    if (!KGlobal::hasMainComponent()) {
        tmp_instance = new KComponentData("nonKDEapp");
    }
    KConfig *config = new KConfig("kcmmidirc");

    KConfigGroup group = config->group("Configuration");
    default_dev = group.readEntry("midiDevice",0);
    if ( default_dev < 0 )
       default_dev=0;
    QString mapurl(group.readPathEntry("mapFilename", QString()));
    if ((group.readEntry("useMidiMapper", false))&&(!mapurl.isEmpty()))
    {
      mapper_tmp = new MidiMapper( mapurl.mid(mapurl.indexOf(":")+1 ).toLocal8Bit().constData() );
    }
    else
      mapper_tmp = 0L;
