...
kmidClient::kmidClient(QWidget *parent, KActionCollection *ac)
    :  QWidget(parent)
{
    ...
    KSharedConfig::Ptr kconf=KGlobal::mainComponent().config();

    KConfigGroup kmidGroup = kconf->group("KMid");
    KConfigGroup midimapperGroup = kconf->group("Midimapper");

    QString tmp2 = KStandardDirs::locateLocal("appdata", "collections");
    collectionsfile = kmidGroup.readPathEntry("CollectionsFile",tmp2);
    slman=new SLManager();
    ...
    timebar->setMaximum(240000);
    timebar->setPageStep(60000);
    timebar->setSingleStep(30000);
    timebar->setValue(0);
    ...
    qlabelTempo= new QLabel(i18n("Tempo:"), this);
    ...
    comboSongs = new QComboBox( this );
    comboSongs->setObjectName( "Songs" );
    comboSongs->setEditable( false );
    connect (comboSongs,SIGNAL(activated(int)),this,SLOT(slotSelectSong(int)));
    comboSongs->setMinimumWidth(200);

    comboEncodings = new QComboBox( this );
    comboEncodings->setObjectName( "Encodings" );
    comboEncodings->setEditable( false );
    connect (comboEncodings,SIGNAL(activated(int)),this,SLOT(slotSelectEncoding(int)));
    comboEncodings->addItem(i18n("Default"));
    comboEncodings->addItems( KGlobal::charsets()->descriptiveEncodingNames() );
    comboEncodings->setCurrentIndex(0);

    rhythmview= new RhythmView( this, "RhythmView");
    rhythmview->setMaximumHeight(7);
    rhythmview->setMinimumHeight(7);

    volumebar = new QSlider(Qt::Vertical, this);
    volumebar->setMinimum(0);
    volumebar->setMaximum(200);
    volumebar->setSingleStep(10);
    volumebar->setPageStep(20);
    volumebar->setValue(100);
    volumebar->setTickPosition(QSlider::NoTicks);
    volumebar->setTickInterval(50);
    connect (volumebar,SIGNAL(valueChanged(int)),this,SLOT(slotSetVolume(int)));

    visiblevolumebar=cfg.readEntry("ShowVolumeBar",0);
    ...
    samplefile.truncate(samplefile.lastIndexOf('/'));
    ...
    int mididev = kmidGroup.readEntry("MidiPortNumber",-1);
    ...
    QByteArray qs=QFile::encodeName(midimapperGroup.readPathEntry("Loadfile","gm.map"));
    ...
    setActiveCollection(kmidGroup.readEntry("ActiveCollection",0));
...
int kmidClient::openFile(const char *filename)
{
    ...
    if ((r=player->loadSong(filename))!=0)
    {
        ...
        {
        case (-1) : errormsg =
                            i18n("The file %1 does not exist or cannot be opened.", filename);
        break;
        case (-2) : errormsg =
                            i18n("The file %1 is not a MIDI file.", filename);break;
        case (-3) : errormsg =
        ...
        case (-6) : errormsg =
                            i18n("%1 is not a regular file.", filename);break;
        ...
        kdispt->ClearEv();
        kdispt->repaint();
        topLevelWidget()->setWindowTitle("KMid");
...
//    kdispt->updateScrollBars();
    ...
    delete [] fn;
    topLevelWidget()->setWindowTitle(capt);
    delete [] capt;
...
int kmidClient::openUrl(const QString _url)
{
	...
	KIO::Job *iojob = KIO::copy( u, KUrl( filename ) );
	downloaded=false;
	connect( iojob, SIGNAL( result( KJob *) ), this, SLOT(downloadFinished( KJob * ) ) );
    ...
    QByteArray filename_8bit = QFile::encodeName(filename);
    int r=-1;
    if (!filename_8bit.isEmpty())
    {
        r=openFile(filename_8bit.data());

        KSharedConfig::Ptr cfg = KGlobal::mainComponent().config();
        KConfigGroup defaultGroup = cfg->group("<default>");

        if (defaultGroup.readEntry("deleteTmpNonLocalFiles",false))
        {
...
void kmidClient::slotPlay()
{
    ...
    if (type!=0) {
        timer4events->setSingleShot(true);
        timer4events->start(x);
    }
...
void kmidClient::slotSeek(int i)
{
    ...
    if (type!=0) {
        timer4events->setSingleShot(true);
        timer4events->start(x-(currentmillisec-beginmillisec));
    }
...
void kmidClient::slotPrevSong()
{
    ...
    comboSongs->setCurrentIndex(currentsl->getActiveSongID()-1);
...
void kmidClient::slotRewind()
{
    ...
    {
        timebar->triggerAction(QAbstractSlider::SliderPageStepSub);
...
void kmidClient::slotForward()
{
    if ((m_kMid.pctl->playing)&&(!m_kMid.pctl->paused))
    {
        timebar->triggerAction(QAbstractSlider::SliderPageStepAdd);
        slotSeek(timebar->value());
    }
}
...
//#endif
    ...
    if (delaymillisec!=~(long)0) {
        timer4events->setSingleShot(true);
        timer4events->start(delaymillisec);
    }
...
void kmidClient::setMidiMapFilename(const char *mapfilename)
{
    ...
    if (map->ok()==-1)
    {
        QString tmp = KStandardDirs::locate("appdata", QString("maps/") + mapfilename);
        delete map;
        map=new MidiMapper(tmp.toLocal8Bit().constData());
        if (map->ok()!=1)
        {
            delete map;
...
{
    activecollection=i;
    KSharedConfig::Ptr kconf=KGlobal::config();

    KConfigGroup kmidGroup = kconf->group("KMid");
    kmidGroup.writeEntry("ActiveCollection",activecollection);
    currentsl=slman->getCollection(activecollection);
    generateCPL();
    initializing_songs=1;
    fillInComboSongs();
...
void kmidClient::fillInComboSongs(void)
{
	...
	//KUrl::decode(qs);
        sprintf(temp,"%d - %s",currentsl->getIteratorID(),
                extractFilename(KUrl::fromPercentEncoding(qs.toAscii()).toAscii().constData(),temp2));
        comboSongs->addItem(temp);
...
void kmidClient::visibleChannelView(int i)
{
	...
	connect(kapp,SIGNAL(aboutToQuit()),parentWidget(),SLOT(shuttingDown()));
...
void kmidClient::downloadFinished(KJob *)
{
    downloaded=true;
    QEventLoop loop(this);
    loop.quit();
...
{
  getComboSongs()->setCurrentIndex(i-1);
  slotSelectSong(i-1);
}
...
void kmidClient::slotSelectEncoding(int i)
{
  if (i == 0)
     ...
     kdispt->setLyricsEncoding(KGlobal::charsets()->encodingForName(comboEncodings->itemText(i)));
}
