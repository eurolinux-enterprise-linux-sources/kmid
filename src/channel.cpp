...
KMidChannel::KMidChannel(int chn,QWidget *parent) : QWidget (parent)
{
    ...
    penT = new QPen (palette().color( QPalette::Background ) );

    KSharedConfig::Ptr kcfg=KGlobal::mainComponent().config();
    KConfigGroup kmidGroup = kcfg->group("KMid");
    QFont *qtextfontdefault=new QFont("lucida",18,QFont::Bold,TRUE);
    qcvfont = new QFont(kmidGroup.readEntry("ChannelViewFont",*qtextfontdefault));
    delete qtextfontdefault;

    instrumentCombo = new QComboBox(this );
    instrumentCombo->setEditable( false );
    instrumentCombo->setObjectName("instr");
    instrumentCombo->setGeometry(160,2,200,20);

    for (i=0;i<128;i++)
        instrumentCombo->addItem(i18n(instrumentName[i]));
    ...
    forcepgm->setCheckable(true);
    button1 = QPixmap(KStandardDirs::locate("appdata","icons/button1.xpm"));
    button2 = QPixmap(KStandardDirs::locate("appdata","icons/button2.xpm"));
...
void KMidChannel::paintEvent( QPaintEvent * )
{
    ...
    QString tmp = i18n("Channel %1", channel);
...
{
    instrumentCombo->setCurrentIndex(pgm);
}

void KMidChannel::changeForceState(bool i)
{
        replay=FALSE;
        forcepgm->setChecked(i);
        replay=TRUE;
}
...
void KMidChannel::reset(int level)
{
    ...
    if (level>=1)
    {
        instrumentCombo->setCurrentIndex(0);
        replay=FALSE;
        forcepgm->setChecked(FALSE);
        replay=TRUE;
    };
...
void KMidChannel::saveState(bool *p,int *pgm)
{
    ...
    *pgm=instrumentCombo->currentIndex();
}
