...
KDisplayText::KDisplayText(QWidget *parent,const char *name) : Q3ScrollView(parent,name)
{
  ...
  QPalette palette;
  palette.setColor(viewport()->backgroundRole(), QColor(110,110,110));
  viewport()->setPalette(palette);
//  setBackgroundMode(NoBackground);
  KSharedConfigPtr kcfg=KGlobal::config();
  KConfigGroup kmidGroup = kcfg->group("KMid");
  typeoftextevents = kmidGroup.readEntry("TypeOfTextEvents",1);
  QFont *qtextfontdefault=new QFont(KGlobalSettings::fixedFont().family(),22);
  qtextfont = new QFont(kmidGroup.readEntry("KaraokeFont",*qtextfontdefault));
...
void KDisplayText::calculatePositions(void)
{
	...
	// problem with latest released Xft
                    tmp->r.translate(tmpx,tmpy-tmp->r.height()-tmp->r.y()-5);
...
void KDisplayText::drawContents(QPainter *p, int /*clipx*/, int clipy, int /*clipw*/, int cliph)
{
    ...
    while ((i<nlinestodraw)&&(tmpl!=NULL))
    {
	    ...
	    p->setPen(Qt::red);
	    p->drawRect(tmp->r);
 	    p->setPen((colorplayed)?Qt::yellow:Qt::black);
#endif
            tmp=tmp->next;
            while ((tmp!=NULL)&&(tmp->spev->type!=typeoftextevents)) tmp=tmp->next;
        }
...
{
/*    KConfig *kcfg=KGlobal::mainComponent().config();
    kcfg->setGroup("KMid");
    typeoftextevents=kcfg->readEntry("TypeOfTextEvents",1);
*/
...
void KDisplayText::setLyricsEncoding(const QString &enc)
{
  ...
  if (enc.isEmpty())
    ...
    newcodec=QTextCodec::codecForName(enc.toLatin1());
